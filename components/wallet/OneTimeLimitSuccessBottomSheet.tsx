...
interface OneTimeLimitSuccessBottomSheetProps {
  visible: boolean;
  amount: number;
  onClose: () => void;
}

export function OneTimeLimitSuccessBottomSheet({
  visible,
  amount,
  onClose,
}: OneTimeLimitSuccessBottomSheetProps) {
  useEffect(() => {
    if (visible) {
        ...
        <TouchableOpacity activeOpacity={1} onPress={() => {}}>
          <View className="items-center rounded-t-3xl bg-white px-6 pb-8 pt-4">
            ...
            <Text className="mb-8 text-center text-base text-grey-plain-550">
              Your one-time transaction limit is now {formattedAmount}. We will
